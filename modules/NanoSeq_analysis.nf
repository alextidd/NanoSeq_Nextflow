...
process COV {
    ...
    // *MODIFIED* (ao7): added flexible memory allocation;
    //                   added task.exitStatus == Integer.MAX_VALUE;
    ...
    memory { ( task.exitStatus == 130 || task.exitStatus == Integer.MAX_VALUE ) ? 300.MB * task.cpus * task.attempt : 300.MB * task.cpus }
    //
...
process PART {
    ...
    memory { ( task.exitStatus == 130 || task.exitStatus == Integer.MAX_VALUE ) ? 9.GB * task.attempt : 9.GB }
    //

    script :
        //optional arguments
        // *MODIFIED* (ao7): handling excludeCov_arg within the bash script
        //def excludeCov_arg = excludeCov != 0 ? "--excludeCov $excludeCov" : ''
        def excludeCov_arg = excludeCov
        //
        ...
        ## *MODIFIED* (ao7): if excludeCov=0, calculate 98th cov percentile (100-bp bins)
        ## The default version of Python3 accessed by Nextflow does not have numpy installed;
        ## using the installation in the path below (FARM5)
        ## PYTHON3=/software/isg/languages/Python-3.9.10/bin/python3
        #/usr/bin/python3 --version; echo `which runNanoSeq.py`; echo `which covStats.py`
        ## [ *CURRENTLY DISABLED* - uncomment lines below to enable ]
        #export PYTHONPATH=/nfs/users/nfs_a/ao7/.local/lib/python3.6/site-packages  # needed for numpy
        #COVSTATS=`which covStats.py`
        #runNanoSeq.py -R ${ref}/genome.fa -A $normal -B $duplex --out $workDir/nanotemps/$meta.id part -n $np $excludeCov_arg $excludeBED_arg;
        if [ "$excludeCov_arg" -eq "0" ]; then
            ## [ *CURRENTLY DISABLED* - uncomment lines below to enable ]
            #cov98=`python3 \$COVSTATS --cov $workDir/nanotemps/$meta.id/tmpNanoSeq/cov | grep "98ile" | cut -d' ' -f3 | cut -d. -f1`
            #echo "Using excludeCov = 98th coverage percentile = \$cov98"
            #runNanoSeq.py -R ${ref}/genome.fa -A $normal -B $duplex --out $workDir/nanotemps/$meta.id part -n $np $excludeBED_arg --excludeCov \$cov98;
            runNanoSeq.py -R ${ref}/genome.fa -A $normal -B $duplex --out $workDir/nanotemps/$meta.id part -n $np $excludeBED_arg;
        else
            echo "Using excludeCov = $excludeCov_arg"
            runNanoSeq.py -R ${ref}/genome.fa -A $normal -B $duplex --out $workDir/nanotemps/$meta.id part -n $np $excludeBED_arg --excludeCov $excludeCov_arg;
        fi
...
process DSA {
    ...
    memory { ( task.exitStatus == 130 || task.exitStatus == 140 || task.exitStatus == Integer.MAX_VALUE ) ? 2.GB * task.attempt : 2.GB }
...
process VAR {
    ...
    memory { ( task.exitStatus == 130 || task.exitStatus == Integer.MAX_VALUE ) ? 3.GB * task.attempt : 3.GB }
    //
...
process POST {
    ...
    memory { ( task.exitStatus == 130 || task.exitStatus == Integer.MAX_VALUE ) ? 5.GB * task.attempt : 5.GB }
    //
